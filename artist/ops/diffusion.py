...
class GaussianDiffusion_style(object):
    ...
    def p_sample(self, xt, t, model, model_kwargs={}, clamp=None, percentile=None, condition_fn=None, guide_scale=None):
        ...
        # random sample (with optional conditional function)
        noise = torch.randn_like(xt)
        t_mask = t.ne(0).float().view(-1, *((1, ) * (xt.ndim - 1)))  # no noise when t == 0
        if condition_fn is not None:
            grad = condition_fn(xt, self._scale_timesteps(t), **model_kwargs)
            mu = mu.float() + var * grad.float()
    ...
    def p_mean_variance(self, xt, t, model, model_kwargs={}, clamp=None, percentile=None, guide_scale=None):
        ...
        if guide_scale is None:
            ...
            if guide_scale != 1.0:
                u_out = model(xt, t=self._scale_timesteps(t), **model_kwargs[1])
                dim = y_out.size(1) if self.var_type.startswith('fixed') else y_out.size(1) // 2
                out = torch.cat([
                    u_out[:, :dim] + guide_scale * (y_out[:, :dim] - u_out[:, :dim]),
                    y_out[:, dim:]], dim=1)
        ...
        if percentile is not None:
            assert percentile > 0 and percentile <= 1  # e.g., 0.995
            s = torch.quantile(x0.flatten(1).abs(), percentile, dim=1).clamp_(1.0).view(-1, 1, 1, 1, 1)
    ...
    @torch.no_grad()
    def ddim_sample(self, xt, t, t_prev, model, model_kwargs={}, clamp=None, percentile=None, condition_fn=None, guide_scale=None, ddim_timesteps=20, eta=0.0):
        r"""Sample from p(x_{t-1} | x_t) using DDIM.
            - condition_fn: for classifier-based guidance (guided-diffusion).
            - guide_scale: for classifier-free guidance (glide/dalle-2).
        """
        ...
        xt_1 = torch.sqrt(alphas_prev) * x0 + direction + t_mask * sigmas * noise
        return xt_1.type(dtype), x0.type(dtype)
    ...
    def ddim_sample_loop(self, noise, model, model_kwargs={}, clamp=None, percentile=None, condition_fn=None, guide_scale=None, ddim_timesteps=20, eta=0.0):
        ...
        for i, step in enumerate(steps):
            t = torch.full((b, ), step, dtype=torch.long, device=xt.device)
            t_prev = torch.full((b, ), steps[i + 1] if i < len(steps) - 1 else 0, dtype=torch.long, device=xt.device)
            xt, _ = self.ddim_sample(xt, t, t_prev, model, model_kwargs, clamp, percentile, condition_fn, guide_scale, ddim_timesteps, eta)
        return xt
    
    @torch.no_grad()
    def ddim_reverse_sample(self, xt, t, t_next, model, model_kwargs={}, clamp=None, percentile=None, guide_scale=None, ddim_timesteps=20):
        r"""Sample from p(x_{t+1} | x_t) using DDIM reverse ODE (deterministic).
        """
        dtype = xt.dtype
        ...
        eps = (_i(self.sqrt_recip_alphas_cumprod, t, xt) * xt - x0) / \
              _i(self.sqrt_recipm1_alphas_cumprod, t, xt)
        alphas_next = _i(torch.cat([self.alphas_cumprod, self.alphas_cumprod.new_zeros([1])]), t_next, xt)
    ...
    def ddim_reverse_sample_loop(self, x0, model, model_kwargs={}, clamp=None, percentile=None, guide_scale=None, ddim_timesteps=20):
        ...
        # reconstruction steps
        steps = (1 + torch.arange(0, self.num_timesteps, self.num_timesteps // ddim_timesteps)).clamp(0, self.num_timesteps - 1)
        for i, step in enumerate(steps):
            t = torch.full((b, ), steps[i - 1] if i > 0 else 0, dtype=torch.long, device=xt.device)
            t_next = torch.full((b, ), step, dtype=torch.long, device=xt.device)
            xt, _ = self.ddim_reverse_sample(xt, t, t_next, model, model_kwargs, clamp, percentile, guide_scale, ddim_timesteps)
        return xt
    
    @torch.no_grad()
    def plms_sample(self, xt, t, t_prev, model, model_kwargs={}, clamp=None, percentile=None, condition_fn=None, guide_scale=None, plms_timesteps=20):
        r"""Sample from p(x_{t-1} | x_t) using PLMS.
            - condition_fn: for classifier-based guidance (guided-diffusion).
            - guide_scale: for classifier-free guidance (glide/dalle-2).
        """
    ...
    def plms_sample_loop(self, noise, model, model_kwargs={}, clamp=None, percentile=None, condition_fn=None, guide_scale=None, plms_timesteps=20):
        ...
        for i, step in enumerate(steps):
            ...
            t = torch.full((b, ), step, dtype=torch.long, device=xt.device)
            t_prev = torch.full((b, ), steps[i + 1] if i < len(steps) - 1 else 0, dtype=torch.long, device=xt.device)
            xt, _, eps = self.plms_sample(xt, t, t_prev, model, model_kwargs, clamp, percentile, condition_fn, guide_scale, plms_timesteps, eps_cache)
    ...
    @torch.no_grad()
    def dpm_solver_sample_loop(self, noise, model, model_kwargs={}, order=2, skip_type='logSNR', method='multistep', clamp=None, percentile=None, condition_fn=None, guide_scale=None,
                               dpm_solver_timesteps=20, algorithm_type='dpmsolver++', t_start=None, t_end=None, lower_order_final=True, denoise_to_zero=False, solver_type='dpmsolver'):
    ...
    @torch.no_grad()
    def inpaint_p_sample(self, xt, t, y, mask, model, model_kwargs={}, clamp=None, percentile=None, guide_scale=None):
    ...
    @torch.no_grad()
    def inpaint_p_sample_loop(self, noise, y, mask, model, model_kwargs={}, clamp=None, percentile=None, guide_scale=None):
        ...
        for step in torch.arange(self.num_timesteps).flip(0):
            t = torch.full((b, ), step, dtype=torch.long, device=xt.device)
            xt, _ = self.inpaint_p_sample(xt, t, y, mask, model, model_kwargs, clamp, percentile, guide_scale)
        return xt
    
    @torch.no_grad()
    def inpaint_mcg_p_sample(self, xt, t, y, mask, model, model_kwargs={}, clamp=None, percentile=None, guide_scale=None, mcg_scale=1.0):
    ...
    @torch.no_grad()
    def inpaint_mcg_p_sample_loop(self, noise, y, mask, model, model_kwargs={}, clamp=None, percentile=None, guide_scale=None, mcg_scale=1.0):
        ...
        for step in torch.arange(self.num_timesteps).flip(0):
            t = torch.full((b, ), step, dtype=torch.long, device=xt.device)
            xt, _ = self.inpaint_mcg_p_sample(xt, t, y, mask, model, model_kwargs, clamp, percentile, guide_scale, mcg_scale)
        return xt

    def loss(self, x0, t, model, model_kwargs={}, noise=None, input_x0=None, reduction='mean'):
        ...
        if self.loss_type in ['kl', 'rescaled_kl']:
            ...
            if self.var_type in ['learned', 'learned_range']:
                out, var = out.chunk(2, dim=1)
                frozen = torch.cat([out.detach(), var], dim=1)  # learn var without affecting the prediction of mean
                loss_vlb, _ = self.variational_lower_bound(x0, xt, t, model=lambda *args, **kwargs: frozen, reduction=reduction)
                if self.loss_type.startswith('rescaled_'):
                    loss_vlb = loss_vlb * self.num_timesteps / 1000.0
    ...
    def variational_lower_bound(self, x0, xt, t, model, model_kwargs={}, clamp=None, percentile=None, reduction='mean'):
        assert reduction in ['mean', 'none']
        ...
        # compute discretized NLL loss (for p(x0 | x1) only)
        nll = -discretized_gaussian_log_likelihood(x0, mean=mu2, log_scale=0.5 * log_var2) / math.log(2.0)
        if reduction == 'mean':
